...
def evaluateModel(config,rep):
    ...
    if batchsize == 3: 
        batchsize = 23 #training samples are 46 only other divisor next two 2,1
        config['batchsize'][0] = 23
    ...
    for x_t,x_v,x_te,y_t,y_v,y_te,o_t,o_v,o_te,x_s_t,x_s_v,x_s_te,y_s_t,y_s_v,y_s_te in data:
        ...
        if os.path.isfile('Models/uber_'+str(state)+'_'+str(rep)+'.h5'):
            model = load_model('Models/uber_'+str(state)+'_'+str(rep)+'.h5')
        else:
            for i in range(epochs):
                model.reset_states()
                model.fit(x_t, y_t, epochs=1, batch_size=batchsize,shuffle = False, verbose=0)
            model = uber_adjust_batchsize(model,config)
            model.save('Models/uber_'+str(state)+'_'+str(rep)+'.h5')
        state = state+1
        model.predict(x_t,batch_size = 1)
        res_v = model.predict(x_v,batch_size = 1)
        #res_v = evaluateUber(res_v,y_v,y_s_v,o_v)
        #print('Val: '+str(res_v))
        #print('Val_sqrt: '+str(sqrt(res_v)))
